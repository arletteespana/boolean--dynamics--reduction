...
def _parse_rule(rule: str, symbols: dict[str, sp.Symbol]) -> sp.Basic:
    ...
    for token in _tokenize_rule(rule):
        ...
        if token in ("!", "~") or low == "not":
            ...
            raise ValueError(f"Unknown token in Boolean rule: {token!r}\nRule: {rule}")

    local_dict = {symbol.name: symbol for symbol in symbols.values()}
    local_dict["_TRUE"] = sp.true
    local_dict["_FALSE"] = sp.false

    return sp.sympify(" ".join(converted), locals=local_dict, evaluate=False)
...
def load_bnet(path: str | Path) -> BNetModel:
    ...
    for line_number, raw_line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        if "," not in line:
            raise ValueError(f"Invalid .bnet line {line_number}: {raw_line}")
    ...
    if len(nodes) != len(set(nodes)):
        raise ValueError("Duplicated target variables were found in the .bnet file.")
...
def write_bnet(
    ...
    for node, expr in output_rules:
        lines.append(f"{node}, {_expr_to_bnet(expr, symbol_names)}")
...
def interaction_graph(model: BNetModel) -> nx.DiGraph:
    graph = nx.DiGraph()
    ...
    for target in model.nodes:
        for symbol in model.rules[target].free_symbols:
            ...
            graph.add_edge(regulator, target)
...
# A dominant set is a feedback vertex set: G - U must be acyclic.
# -----------------------------------------------------------------------------

def _cycle_nodes(graph: nx.DiGraph) -> list[str] | None:
    try:
...
def _disjoint_cycle_lower_bound(graph: nx.DiGraph) -> int:
    work = graph.copy()
    count = 0
...
def dominant_depth(graph: nx.DiGraph, dominant_set: set[str]) -> int:
    ...
    while len(known) < graph.number_of_nodes():
        new_nodes = {
            node
